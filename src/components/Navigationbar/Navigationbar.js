...
const NavigationBar = (props) => {   
    ...
    if (config.categories) {
        categoriesList = config.categories.map((category, index) => (
            <NavDropdown.Item 
                ...
                onClick={() => props.filterByCategories(category.displayText)} 
                href={"#" + category.value}>
    ...
    return (
        <Navbar expand="lg" className={styles.NavBar}>
            <Container>
                <Navbar.Brand className={styles.NavBarBrand} onClick={() => props.filterByCategories('')} href="#home">Bootstrap Themes</Navbar.Brand>
                ...
                <Navbar.Collapse id="navbar-collapse">
                    <Form className={formNavClasses.join(" ")} inline>
                        <FormControl type="text" placeholder="Search" value={props.searchText} onChange={(event) => props.searchThemes(event)} />
                    </Form>
